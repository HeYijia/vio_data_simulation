...
int main()
{
    ...
    intrinsic_matrix << 458.654, 0.0000000000000000e+00, 367.215,
    ...
    for (int n = 0; n < camdata.size(); ++n)
    {
        ...
        for (int i = 0; i < points.size(); ++i)
        {
            ...
            Eigen::Vector3d pc_real = intrinsic_matrix * Eigen::Vector3d(pc1(0), pc1(1), pc1(2));

            if (pc1(2) < 0)
                continue; // z必须大于０,在摄像机坐标系前方

            Eigen::Vector2d obs(pc1(0) / pc1(2), pc1(1) / pc1(2));
            Eigen::Vector2d obs_real(pc_real(0) / pc_real(2), pc_real(1) / pc_real(2));
            ...
            // (obs(1)*460 + 255) > 0 && ( obs(1)* 460 + 255) < params.image_w )
            {
                ...
                features_cam.push_back(obs_real);
            }
        }
